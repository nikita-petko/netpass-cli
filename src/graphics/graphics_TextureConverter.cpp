...
#define SWAP(A, B) \
	...
	static s32 ReadBlock(u32 no, u8* buf, TextureInformation* textureInformation)
	{
		...
		if (textureInformation->mode == TEXTURE_BLOCK_TO_LINE_MODE)
		{
			...
			const u8* pSrc = textureInformation->src + no * pixelByte * TEXTURE_PIXELS_IN_BLOCK;
		...
		{
			/* Get source buffer pointer */
			const u8* pSrc = textureInformation->src + (row * TEXTURE_PIXELS_IN_BLOCK * pixelByte * textureInformation->blocksInRow) +
	...
	static s32 WriteBlock(u32 no, const u8* pSrc, TextureInformation* textureInformation)
	{
		...
		if (textureInformation->mode == TEXTURE_BLOCK_TO_LINE_MODE)
		{
			...
			{
				pDst =
					textureInformation->dst +
					((textureInformation->blocksInCol - row - 1) * TEXTURE_PIXELS_IN_BLOCK * pixelByte * textureInformation->blocksInRow) +
					column * pixelByte * TEXTURE_BLOCK_SIZE;
			}
			else
			{
				pDst = textureInformation->dst + (row * TEXTURE_PIXELS_IN_BLOCK * pixelByte * textureInformation->blocksInRow) +
					   column * pixelByte * TEXTURE_BLOCK_SIZE;
			}
		...
		else  // if (mode == L2B)
		{
			...
			if (textureInformation->yflip)
			{
				pDst = textureInformation->dst + ((textureInformation->blocksInCol - row - 1) * textureInformation->blocksInRow + column) *
													 TEXTURE_PIXELS_IN_BLOCK * pixelByte;
			}
			else
			{
				pDst = textureInformation->dst + no * TEXTURE_PIXELS_IN_BLOCK * pixelByte;
			}
	...
	static s32 DeBlock(const u8* pSrc, u8* pDst, s32 blocksize, TextureInformation* textureInformation)
	{
		...
		if (blocksize != 1)
		{
			...
			if (!textureInformation->yflip)
			{
				DeBlock(pSrc + (pixelByte * pixels * 0), pDst, blocksize, textureInformation);

				DeBlock(pSrc + (pixelByte * pixels * 1), pDst + (pixelByte * blocksize), blocksize, textureInformation);
			...
			else
			{
				...
				DeBlock(pSrc + (pixelByte * pixels * 2), pDst, blocksize, textureInformation);

				DeBlock(pSrc + (pixelByte * pixels * 3), pDst + (pixelByte * blocksize), blocksize, textureInformation);
			}
		}
		else
	...
	static s32 EnBlock(const u8* pSrc, u8* pDst, s32 blocksize, TextureInformation* textureInformation)
	{
		...
		if (blocksize != 1)
		{
			...
			if (!textureInformation->yflip)
			{
				EnBlock(pSrc, pDst + (pixelByte * pixels * 0), blocksize, textureInformation);

				EnBlock(pSrc + (pixelByte * blocksize), pDst + (pixelByte * pixels * 1), blocksize, textureInformation);
			...
			{
				EnBlock(pSrc, pDst + (pixelByte * pixels * 2), blocksize, textureInformation);

				EnBlock(pSrc + (pixelByte * blocksize), pDst + (pixelByte * pixels * 3), blocksize, textureInformation);
