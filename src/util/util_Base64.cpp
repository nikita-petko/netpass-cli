...
const char s_Base64Chars[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+-";
...
namespace np { namespace util {
	...
	nn::Result Base64Encode(char* pOut, size_t* outSize, const void* pIn, size_t inSize)
	{
		...
		{
			pOut[j + 0] = s_Base64Chars[(pInBytes[i + 0] & 0xFC) >> 2];
			pOut[j + 1] = s_Base64Chars[((pInBytes[i + 0] & 0x03) << 4) | ((pInBytes[i + 1] & 0xF0) >> 4)];
			pOut[j + 2] = s_Base64Chars[((pInBytes[i + 1] & 0x0F) << 2) | ((pInBytes[i + 2] & 0xC0) >> 6)];
			pOut[j + 3] = s_Base64Chars[pInBytes[i + 2] & 0x3F];
		}
